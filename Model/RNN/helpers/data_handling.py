...
class DataHandler(object):
    ...
    def _get_path(self, dirname):
        ...
        if os.path.exists(dirname) and os.path.exists(DEFAULT_DIR + dirname + '/'):
            print(
                'WARNING: ambiguous directory name, both "' + dirname + '" and "' + DEFAULT_DIR + dirname + '" exist. "' + dirname + '" is used.')
            return dirname
    ...
    def _load_stats(self):
        ...
        with open(self.dirname + 'data/stats', 'r') as f:
            ...
            stats = f.readline().split()[1:]
            self.training_set.n_users, self.training_set.n_items, self.training_set.n_interactions, self.training_set.longest_sequence = map(
                int, stats[0:4])
            ...
            stats = f.readline().split()[1:]
            self.validation_set.n_users, self.validation_set.n_items, self.validation_set.n_interactions, self.validation_set.longest_sequence = map(
                int, stats[0:4])
            ...
            stats = f.readline().split()[1:]
            self.test_set.n_users, self.test_set.n_items, self.test_set.n_interactions, self.test_set.longest_sequence = map(
                int, stats[0:4])
            ...
            if self.extended_training_set:
                ...
                self.training_set.n_interactions += (
                                                                self.validation_set.n_interactions + self.test_set.n_interactions) / 2
    ...
    def __call__(self, min_length=2, max_length=None, length_choice='max', subsequence='contiguous', epochs=np.inf):
        ...
        while counter < epochs:
            ...
            for j, sequence in enumerate(self.lines):
                ...
                if "-" not in sequence[2]:  # no timestamp in sequence
                    sequence = [[int(sequence[2 * i]), float(sequence[2 * i + 1])] for i in
                               range(int(len(sequence) / 2))]
                else:  # with timestamp in sequence
                    sequence = [[int(sequence[3 * i]), float(sequence[3 * i + 1])] for i in range(int(len(sequence) / 3))]
