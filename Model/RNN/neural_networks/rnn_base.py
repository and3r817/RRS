...
class RNNBase(object):
        ...
        try:
            while time() - start_time < max_time and iterations < max_iter:
                ...
                try:
                    ...
                    if self.framework == 'tf':
                        if self.save_log:
                            s, cost, _ = self.sess.run([self.summary, self.cost, self.training],
    ...
    #                 # )
    #                 seq_lengths = sorted(
    #                     random.sample(range(2, min(len(sequence), self.max_length)),  # range, min_length = 2
    #                                   min([batch_size - j, min(len(sequence), self.max_length)-2])), reverse=True
    #                 )
    #             elif self.iter:
    #                 batch_size = len(sequence) - 1
    #                 seq_lengths = list(range(1, len(sequence)))
