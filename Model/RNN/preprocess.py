...
def command_parser():
    ...
    parser.add_argument('-f', dest='filename', help='Input file',
                        default='/Users/xun/Documents/Thesis/Improving-RNN-recommendation-model/Dataset/ks-cooks-1y-v2/ks-cooks-1y.csv',
                        type=str)
    parser.add_argument('--columns',
                        help='Order of the columns in the file (eg: "uirt"), u for user, i for item, t for timestamp, r for rating. '
                             ...
                             'are assumed to be in chronological order. Extra columns are ignored. Default: uit',
    ...
    parser.add_argument('--max_item_pop',
                        help='Items with more interactions than this will be removed from the dataset. Default: 850',
...
def remove_rare_elements(data, min_user_activity, min_item_popularity, max_item_popularity, min_time_difference):
    ...
    # Remove blockbuster items
    item_popularity = data.groupby('i').size()
    data = data[np.in1d(data.i, item_popularity[item_popularity <= max_item_popularity].index)]
...
def split_data(data, nb_val_users, nb_test_users, dirname, save_time=True):
...
def main():
    ...
    data = remove_rare_elements(data, args.min_user_activity, args.min_item_pop, args.max_item_pop, args.min_time_diff )
